...
ContextObject 存储的是观察（observations），不是解释（interpretations）。每个字段必须满足：可由确定性算法生成、不包含情绪判断、不包含关系判断、不包含面向用户的话术、完全可测试。`memory_summary` 当前保持不变（API Contract），未来大版本升级为 `memory_facts`（纯事实拼接）。

---

*最后一个 commit 写完了吗？拿出来，拿这十条一条一条对。*