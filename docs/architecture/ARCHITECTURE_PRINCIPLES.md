...
ContextObject 存储的是观察（observations），不是解释（interpretations）。每个字段必须满足：可由确定性算法生成、不包含情绪判断、不包含关系判断、不包含面向用户的话术、完全可测试。

## 11. Architecture First, Implementation Incrementally

架构可以一次设计到 v2.0。实现必须按版本渐进完成。任何架构预留都不能假装已经实现，任何当前实现都不能破坏未来架构。

## 12. Future Complexity, Present Simplicity

任何为了未来而增加的能力，都不能增加今天生产环境的复杂度；任何为了今天而做出的简化，都不能阻断未来的架构演进。

---

*最后一个 commit 写完了吗？拿出来，拿这十二条一条一条对。*